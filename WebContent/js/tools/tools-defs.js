	    ...
	    return [
	      ...
	      },
	      {
	          ...
	          description: 'Создаёт и регистрирует новый инструмент в tools-engine. ' +
	            ...
	            'ФАЙЛ ПОЛЬЗОВАТЕЛЮ отдаётся вызовом await agent_download({ name, content, mime }) — ' +
	            'скачивание запускает само приложение, потому что кадру песочницы браузер это запрещает. ' +
	            'Для двоичного файла передай base64:true и содержимое строкой base64. ' +
	            'Привычный Blob + <a download> + a.click() тоже работает: клик перехватывается и файл ' +
            'уходит тем же путём. А вот showSaveFilePicker и window.open в песочнице недоступны. ' +
            'Не объявляй файл сохранённым по своей инициативе: если он не ушёл, в результате ' +
            'вызова появится поле download_error — тогда так и скажи. ' +
	            'ЭКРАННАЯ ФОРМА — await agent_form({ title, description, fields }). Показывает окно ' +
	            'и возвращает { submitted, values }: submitted:false означает, что человек закрыл окно — ' +
	            'тогда ничего не делай и верни это как результат, а не выдумывай значения. ' +
	            'Поле: { name, label, type, value, required, hint }, где hint — пояснение, которое всплывает ' +
	            'при наведении мыши на подпись и на само поле (под полем текстом оно не выводится), а type — text, textarea, number, ' +
	            'select (options), radio (options), checkbox, password, date или info (просто текст). ' +
	            'Полей до тридцати. Форму можно показывать несколько раз за вызов: спросить, сходить ' +
	            'за данными, показать выбор из полученного. Своей вёрстки у формы нет — приложение ' +
	            'рисует её само, чтобы окно инструмента не отличалось от остальных окон. ' +
	            'Время, которое человек провёл в форме, не считается работой инструмента: таймаут ' +
	            'вызова на него не расходуется. У окна есть обратный отсчёт — по умолчанию предел ' +
	            'времени на вызов; свой задаётся полем seconds (0 — ждать без ограничения). Когда ' +
	            'время выходит, окно закрывается ответом onTimeout — задай его, если знаешь разумное ' +
	            'значение по умолчанию; без него истёкший срок приходит как отказ (submitted:false, ' +
	            'timedOut:true). Человек может остановить отсчёт и отвечать без спешки. ' +
	            'ОКНО СО СВОЕЙ ВЁРСТКОЙ — await agent_dialog({ title, width, height }, render), когда ' +
	            'полей мало: таблица с отбором, предпросмотр, схема, холст. Кадр песочницы показывается ' +
	            'модальным окном, и внутри него доступен обычный document: render(done) рисует что угодно ' +
	            'в document.body и вызывает done(значение), когда человек ответил. Возвращается ' +
	            '{ closed, value }: closed:true — человек закрыл окно, это ОТКАЗ, значения не выдумывай. ' +
	            'Пока окно открыто, ход агента стоит: ни таймаут вызова, ни бюджет хода не тратятся. ' +
	            'Обратный отсчёт и здесь тот же: seconds и onTimeout работают так же, как у agent_form. ' +
	            'Пояснения в своей вёрстке делай подсказками при наведении, как во всех окнах приложения: ' +
	            '<label class="hint" title="пояснение">Поле</label> — знак вопроса и курсор добавятся сами. ' +
	            'Разметка живёт в отдельном документе песочницы, до данных приложения из неё не дотянуться; ' +
	            'если хватает обычных полей — бери agent_form, она выглядит как остальные окна. ' +

  ...
  _initBuiltinTools() {
        ...
        const topics = {
          overview: {
            ...
            points: [
              ...
              'Имя агента вы задаёте сами — оно видно в шапке, и агент на него отзывается. Переименовать можно, просто попросив.',
              'История доработок: значок с номером релиза в шапке показывает, что появилось и когда; спросите «что нового» — расскажу.',
            ],
          },
          skills: {
          ...
          limits: {
            ...
            points: [
              ...
              'Окно контекста — сколько переписки помещается в запрос. Когда не помещается, начало не теряется, а сворачивается в резюме.',
              'Большой результат инструмента не занимает контекст целиком: он сохраняется отдельно, а агент читает из него нужные куски.',
              'Длинную черновую работу агент может выполнить подзадачей — отдельным ходом, из которого в чат возвращается только итог. Её переписку видно кнопкой в блоке вызова.',
              'Ход работы над задачей в несколько этапов ведётся планом: он виден в панели чата и переживает перезагрузку страницы.',
              'Если работа оборвалась (закрытая вкладка, сбой), при следующем открытии чата будет предложено продолжить с места остановки.',
    ...
    this.registerHandler('builtin_proxy_fetch', async (params) => {
      try {
        ...
        // Прокси отдаёт сбой проверки сертификата отдельным, разобранным
        // видом (tlsError). Пропустить это как обычное тело ответа значило
        // бы оставить модель гадать по коду 502 — а чинится оно не здесь и
        // не ею: настройку правит пользователь в config.js прокси.
        if (resp.status === 502) {
          let parsed = null;
          try { parsed = JSON.parse(full); } catch (_) { parsed = null; }
          if (parsed && parsed.tlsError) {
            return {
              ...
              hint: 'Это не ошибка запроса и не повод пробовать другой инструмент: сертификат целевого ' +
                    'сервера не проходит проверку у прокси. Передай пользователю варианты из howToFix — ' +
                    'править нужно tls в config.js прокси и перезапустить его.',
            };
          }
        }
    ...
    this.registerHandler('builtin_update_tool', async (params) => {
      try {
        ...
        // Встроенный инструмент поставляется с приложением: его имя,
        // описание, схема параметров и обработчик — часть кода, а не
        // пользовательские данные. Правка описания опаснее, чем кажется:
        // именно по нему модель решает, когда инструмент вызывать.
        // Пользовательское состояние (включён, папка) менять можно.
        const contentFields = ['newName', 'description', 'parameters', 'handlerCode'];
        if (t.builtin && contentFields.some(f => params[f] !== undefined)) {
          return {
            error: 'Инструмент "' + t.name + '" встроенный: его имя, описание, параметры и код изменить нельзя.',
            hint: 'Нужно другое поведение — создай отдельный инструмент через create_tool. ' +
                  'Включение и папку у встроенного менять можно (кроме системных).',
          };
        }

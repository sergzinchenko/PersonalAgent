...
class UI {
  constructor(agent) {
    ...
    // ── Детализация вызовов инструментов ──
    // 'hidden' — в переписку не писать вовсе (по умолчанию), 'compact' —
    // имя и начало результата, 'detailed' — аргументы и полный результат
    // в раскрывающемся блоке.
    //
    // Скрытый режим стал умолчанием не потому, что вызовы неважны, а
    // потому, что переписка — для разговора. Технический протокол работы
    // вытеснял из неё сам разговор: на длинной задаче между двумя
    // репликами человека оказывалось по три десятка служебных блоков.
    // Видимость при этом не потерялась: пока агент работает, ход вызовов
    // показывает лента над полем ввода (см. _renderToolTrack), а после —
    // счётчик в панели чата. Настройка управляет ПЕРЕПИСКОЙ, а не тем,
    // видно ли работу.
    this.toolVerbosity = 'hidden';

    // ── Глубина панели хода ──
    // Отдельная настройка, не связанная с перепиской: держать переписку
    // чистой и при этом подробно следить за работой — не противоречие,
    // а обычное желание. Уровни вложены друг в друга:
    //   'off'      — панели нет, только строка счёта над полем ввода;
    //   'steps'    — шаги плана;
    //   'subtasks' — + подзадачи с их прогрессом;
    //   'tools'    — + вызовы инструментов (по умолчанию);
    //   'io'       — + аргументы и ответы в раскрывающихся строках.
    this.panelDepth = 'tools';
  ...
  _bindSidebarLayout() {
    ...
    toggle?.addEventListener('click', () => {
      // Панель сворачивается целиком, и переписка занимает её место
      // (см. .sidebar-collapsed в styles.css).
...
// Высота заголовка окна инструмента. Число нужно и разметке, и подсчёту
// смещения кадра, поэтому живёт в одном месте, а не в двух похожих.
UI.SANDBOX_DIALOG_HEAD = 38;

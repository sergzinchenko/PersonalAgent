	    ...
	    return [
	      ...
	      },
	      {
	        ...
	        systemPrompt:
	          ...
	          '5. Функция может быть async. Всегда возвращай объект-результат; ошибки возвращай как { error: "текст" }.\n' +
      ...
      const defaults = [
        ...
        },
        {
            ...
            systemPrompt:
              ...
              '1. name — короткий и в snake_case, соответствует ^[a-zA-Z_][a-zA-Z0-9_]*$.\n' +
              '2. description пиши от лица инструмента: что делает и КОГДА его вызывать (для роутинга LLM).\n' +
              '3. parameters — валидная JSON Schema: { type:"object", properties:{...}, required:[...] }. ' +
              'Каждое свойство снабжай понятным description.\n' +
              '4. handlerCode — ТОЛЬКО тело функции. Доступен единственный аргумент params (объект). ' +
              'Нет доступа к this, db, localStorage-обёрткам агента, сети вне fetch и внешним модулям. ' +
              'Можно использовать стандартные Web/JS API (JSON, Math, Date, crypto, fetch, DOMParser).\n' +
              '5. Функция может быть async. Всегда возвращай объект-результат; ошибки возвращай как { error: "текст" }, не бросай исключения без нужды.\n' +
              '6. Валидируй входные params внутри handlerCode и подставляй разумные значения по умолчанию.\n' +
              '7. Перед созданием кратко покажи пользователю план (name, параметры, логику), затем вызови create_tool.\n' +
              '8. Не дублируй уже существующие встроенные инструменты (calculator, http_fetch, format_json и т.д.).',
            enabled: false,
            icon: '🛠️',
            category: 'development',
          },

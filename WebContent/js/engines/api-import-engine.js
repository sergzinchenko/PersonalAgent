...
class ApiImportEngine {
  ...
  // ── Оборванный текст ──
  // Отвечает на вопрос «это описание целиком или его хвост потеряли».
  // Нужен до разбора: у оборванного JSON и у файла в неизвестном формате
  // симптом одинаковый — «не разобралось», а причины и лечение разные.
  // Возвращает причину строкой или null, если текст выглядит целым.
  static looksTruncated(text) {
    const t = String(text || '');
    const trimmed = t.trim();
    if (!trimmed) return 'пустой текст';

    // Явная метка усечения от нашего же чтения (read_file, артефакты).
    if (/…\s*\[(результат|текст) сокращ[её]н/i.test(t.slice(-400))) return 'в конце стоит отметка о сокращении';

    if (trimmed.startsWith('{') || trimmed.startsWith('[')) {
      try { JSON.parse(trimmed); return null; } catch (_) { /* разбираем причину ниже */ }
      const last = trimmed[trimmed.length - 1];
      // Целый JSON заканчивается закрывающей скобкой. Если нет — это не
      // «плохой JSON», а именно хвост, который не доехал.
      if (last !== '}' && last !== ']') return 'JSON обрывается на середине';
      return null;   // скобка на месте — значит, дело не в усечении
    }

    if (trimmed.startsWith('<')) {
      // У XML/WSDL тот же признак: последний тег должен быть закрывающим.
      if (!/<\/[^>]+>\s*$/.test(trimmed) && !/\/>\s*$/.test(trimmed)) return 'XML обрывается на середине';
    }
    return null;
  }

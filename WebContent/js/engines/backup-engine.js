...
class BackupEngine {
  ...
  async _applyStore(store, rows, mode, report, enableImportedCode) {
    ...
    for (const row of rows) {
      ...
      // пользователь: где инструмент лежит и включён ли он. Описание и
      // параметры остаются локальными: они могут быть новее слепка, и
      // именно по ним модель решает, когда инструмент вызвать.
      if (store === 'tools' && rec.builtin) {
        if (!local) { report.skipped++; continue; }
        local.parentId = rec.parentId ?? null;
